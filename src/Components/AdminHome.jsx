...
const AdminHome = () => {
  ...
  // Once admin data is loaded, redirect Lower Admin away from dashboard overview
  useEffect(() => {
    if (!adminData) return;
    const name = (adminData?.fullName || adminData?.name || "")
      .trim()
      .toLowerCase();
    const isLowerAdmin = name === "lower admin";
    if (isLowerAdmin && location.pathname === "/admin/dashboard") {
      navigate("/admin/dashboard/all-chats", { replace: true });
    }
